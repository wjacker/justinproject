			...
			if ( streamDescriptor != null ) {
				...
				if ( _netStreamTable[streamDescriptor.id] == null ) {
					...
					if ( _streamManager.isP2P ){ 
						stream = new NetStream(_connectSession.sessionInternals.session_internal::connection as NetConnection,streamDescriptor.peerID);
						// if we have a peerTimeout and it is not running , then I will start it ....
						if ( _peerTimeoutTable[streamDescriptor.id] == null ) {
						...
						stream= new NetStream(_connectSession.sessionInternals.session_internal::connection as NetConnection,NetStream.CONNECT_TO_FMS);
					}
								
            		_netStreamTable[streamDescriptor.id]= stream; 
