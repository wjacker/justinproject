...
package com.adobe.rtc.collaboration
{
	 ...
	 */	
	 public class WebcamPublisher extends UIComponent implements ISessionSubscriber
	 {
		 ...
		 * @private
		 */
		protected var _video:VideoComponent;
		...
		protected function onConnectionTypeChange(p_evt:StreamEvent):void
		{
			...
			if ( _streamManager.isP2P) {
				...
				setTimeout(sendSnapShotPermission, 2000);
     			...
     			_stream= new NetStream(_connectSession.sessionInternals.session_internal::connection as NetConnection,NetStream.CONNECT_TO_FMS);
     		}
			...
			if( _camera==null){
				...
				if ( _camera.muted || !p_evt.streamDescriptor.finishPublishing )
					...
					if(	streamDescriptor.nativeWidth!=0 && streamDescriptor.nativeHeight!=0) {
            			...
            			if( _stream == null ){
            				_stream= new NetStream(_connectSession.sessionInternals.session_internal::connection as NetConnection);
                			_stream.attachCamera(_camera);
            			} 
