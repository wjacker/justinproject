package com.adobe.rtc.session.managers
{
	 ...
	 * @see com.adobe.rtc.messaging.serveremulation.LocalConnectionMessageManager LocalConnectionMessageManager
	 * @see com.adobe.rtc.messaging.manager.MessageManager MessageManager
	 * 
	 */
	public class SessionManagerLocalConnection extends SessionManagerBase
	{
		/**
		...
		*/
		...
		{
			_outgoingConnection.send(SERVER_REQUEST_CONNECTION_NAME, "pingServer",userID);
			beepTimer.start();
		}	
		...
		override session_internal function login():void
		{
			...
			if ( authenticator.userName != null ) {
				_outgoingConnection.send(SERVER_REQUEST_CONNECTION_NAME, "acceptClient", {role:100,userName:authenticator.userName});
			}else {
				_outgoingConnection.send(SERVER_REQUEST_CONNECTION_NAME, "acceptClient", {role:100});
			}
		...
		override session_internal function subscribeCollection(p_collectionName:String=null, p_nodeNames:Array=null):void
		{
			...
			if (isSynchronized) {
				session_internal::connection.send(SERVER_REQUEST_CONNECTION_NAME, "subscribeCollection", userID,p_collectionName,p_nodeNames);
			} else {
		...
		override session_internal function unsubscribeCollection(p_collectionName:String=null):void
		{
			connection.send(SERVER_REQUEST_CONNECTION_NAME, 
									"unsubscribeCollection", messageManager.connectSession.userManager.myUserID, p_collectionName);
		}
		...
		override session_internal function createNode(p_collectionName:String, p_nodeName:String, p_nodeConfigurationVO:Object=null):void
		{
			_outgoingConnection.send(SERVER_REQUEST_CONNECTION_NAME, 
									"createNode", messageManager.connectSession.userManager.myUserID, p_collectionName, p_nodeName, p_nodeConfigurationVO);
		}
		...
		{
			_outgoingConnection.send(SERVER_REQUEST_CONNECTION_NAME, 
									"configureNode", messageManager.connectSession.userManager.myUserID, p_collectionName, p_nodeName, p_nodeConfigurationVO);
		}
		...
		{
			_outgoingConnection.send(SERVER_REQUEST_CONNECTION_NAME, 
									"removeNode", messageManager.connectSession.userManager.myUserID, p_collectionName, p_nodeName);
		}
		...
		override session_internal function publishItem(p_collectionName:String, p_nodeName:String, p_itemVO:Object, p_overWrite:Boolean=false):void
		{
			session_internal::connection.send(SERVER_REQUEST_CONNECTION_NAME, 
									"publishItem", messageManager.connectSession.userManager.myUserID, p_collectionName, p_nodeName, p_itemVO);
		}
		...
		{
			_outgoingConnection.send(SERVER_REQUEST_CONNECTION_NAME, 
									"retractItem", messageManager.connectSession.userManager.myUserID, p_collectionName, p_nodeName, p_itemID);
		}
		...
		{
			_outgoingConnection.send(SERVER_REQUEST_CONNECTION_NAME, 
									"setUserRole", messageManager.connectSession.userManager.myUserID, p_userID, p_role, p_collectionName, p_nodeName);
		}
