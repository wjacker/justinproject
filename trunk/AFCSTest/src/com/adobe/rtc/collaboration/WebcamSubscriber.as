					...
					if ( streamDescriptor.streamPublisherID != _userManager.myUserID || _isMyPaused) { 
						if ( _netStreamTable[streamDescriptor.id] == null ) {
							if ( _streamManager.isP2P) { 
								_netStreamTable[streamDescriptor.id]= new NetStream(_connectSession.sessionInternals.session_internal::connection as NetConnection,streamDescriptor.peerID);
								...
								_netStreamTable[streamDescriptor.id]= new NetStream(_connectSession.sessionInternals.session_internal::connection as NetConnection,NetStream.CONNECT_TO_FMS);
	            			}
...
//				_netStreamTable[p_stream.id].attachCamera(null);
		...
		protected function pauseStream(p_streamType:String,p_streamPublisherID:String) :void
		{
			...
			if (streamDescriptor.pause) {
				...
				if ( streamDescriptor.streamPublisherID == _userManager.myUserID ) {
					...
					if ( _streamManager.isP2P && _userManager.isPeerEnable()) {
						_netStreamTable[streamDescriptor.id]=new NetStream(_connectSession.sessionInternals.session_internal::connection as NetConnection,streamDescriptor.peerID);
        ...
        */
		...
		protected function onNetStatus(e:NetStatusEvent):void
		{
			...
			if (e.info.code == "NetStream.Play.Start" || e.info.code == "NetStream.Play.PublishNotify" )
			{
			 	var stream:NetStream = e.currentTarget as NetStream ;
